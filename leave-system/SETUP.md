...
## ขั้นตอนที่ 8: ผูกหัวหน้าแผนกเป็นผู้อนุมัติ

หัวหน้าแผนกต้องมี "LineUserId" ในชีต Employees ก่อนถึงจะตั้งเป็นผู้อนุมัติได้ วิธีที่ง่ายที่สุด:

1. เพิ่มเพื่อน LINE OA ของบริษัท (QR code อยู่ในแท็บ Messaging API > QR code)
2. ให้หัวหน้าแผนกเปิดลิงก์ `https://liff.line.me/<LIFF_ID>` แล้วลงทะเบียนตามปกติ (เลือกแผนกตัวเอง)
   ระบบจะสร้างแถวในชีต **Employees** ให้อัตโนมัติ พร้อม `LineUserId` ของเขา
3. เปิดชีต **Employees** คัดลอกค่า `LineUserId` ของหัวหน้าคนนั้น
4. ไปที่ชีต **Departments** วางค่านี้ลงในช่อง `ApproverLineUserId` ของแผนกที่เขาดูแล
5. เสร็จแล้ว — เมื่อลูกทีมในแผนกนั้นยื่นคำขอลา หัวหน้าจะได้รับข้อความ Flex Message
   พร้อมปุ่มอนุมัติ/ไม่อนุมัติทันที (และยังกดอนุมัติผ่านแท็บ "รออนุมัติ" ในแอปได้ด้วย)
...
- **LeaveRequests**: ประวัติคำขอลาทั้งหมด พร้อมสถานะ PENDING / APPROVED / REJECTED
  และ `TimePeriod` (FULL = เต็มวัน, MORNING = ครึ่งวันเช้า, AFTERNOON = ครึ่งวันบ่าย — ลาครึ่งวันหัก 0.5 วัน)
